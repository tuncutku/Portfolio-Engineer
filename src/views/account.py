...
@account_blueprint.route("/delete/<string:portfolio_name>", methods=["GET", "POST", "PUT", "DELETE"])
@requires_login
...
def add_portfolio():
    if request.method == "POST":
        ...
        if not _valide_portfolio_name(name, session["email"]):
            ...
            return render_template("account/add_portfolio.html", error_message = error_message)
        else:
...
def edit_portfolio(portfolio_name):
    ...
    if request.method == "POST":
        ...
        if name == port.name and status == port.status and port_type == port.portfolio_type:
            error_message = "Your current portfolio has the same attributes!"
            return render_template("account/edit_portfolio.html", portfolio = port, error_message = error_message)  
        if not _valide_portfolio_name(name, session["email"]) and name != port.name:
            ...
            return redirect(url_for("account.list_portfolios"))
    return render_template("account/edit_portfolio.html", portfolio = port, error_message = None)
