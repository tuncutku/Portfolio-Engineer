...
class Position:
    ...
    @classmethod
    def find_all(
        cls, portfolio_id: int, position_filter: List[str] = ["Open", "Closed"]
    ) -> List["Position"]:
        position_list = DB_Position.get_positions(portfolio_id)
        return [
            cls(*position)
            for position in position_list
            if position[3] in position_filter
        ]
    ...
    def add_position(self) -> None:
        DB_Position.add_position(
            self.symbol, self.source, self.quantity, self.state, self.portfolio_id
        )
